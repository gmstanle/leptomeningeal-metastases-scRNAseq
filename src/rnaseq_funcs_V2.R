libloc="/Library/Frameworks/R.framework/Versions/3.1/Resources/library"
require(data.table)
require(ggplot2)
require(reshape2)
...
load.count.table <- function(fname, mincount = 1e5,min.num.genes=1e3, omit.bad.genes=TRUE, omit.bad.cells=TRUE,omit.mult.cells=T,numCols=3){
  require(data.table)
  require(reshape2)
  require(ggplot2)
  data.counts <- fread(fname)
  
  if(numCols==7){
    setnames(data.counts,c("cell.name","gene","count","num_cells","num_red","num_green","num_blue")); 
    data.counts[,c("num_cells","num_red","num_green","num_blue")
                :=list(as.numeric(as.character(num_cells)),
                       as.numeric(as.character(num_red)),
                       as.numeric(as.character(num_green)),
                       as.numeric(as.character(num_blue)))]
    setkey(data.counts)
    if(omit.mult.cells){
      data.counts <- data.counts[num_cells==1] 
    }
    cast.counts <- dcast.data.table(data.counts, cell.name+num_cells+num_red+num_green+num_blue~gene, value.var="count",fun.aggregate = mean,fill=0)
    data.counts <- melt(cast.counts,id.vars = 1:5,variable.name = "gene",value.name="count")
  }
  else if (numCols==3){
    setnames(data.counts,c("cell.name","gene","count")); 
    data.counts[,num_cells:=as.numeric(substr(cell.name,1,1))]
    setkey(data.counts)
    if(omit.mult.cells){
      data.counts <- data.counts[num_cells==1] 
    }
    cast.counts <- dcast.data.table(data.counts, cell.name+num_cells~gene, value.var="count",fun.aggregate = mean,fill=0)
    data.counts <- melt(cast.counts,id.vars = c("cell.name","num_cells"),variable.name = "gene",value.name="count")
  }
  cell.counts <- get.mm10.ercc.ratio(data.counts)
  plt=ggplot(cell.counts,aes(x=log10(mm10+1),y=log10(ERCC+1)))+geom_point(alpha=0.7,size=1)+theme_bw()+geom_vline(xintercept=log10(1e5),color="red")+
    ggtitle("mm10 vs ERCC counts\n red line = QC cutoff")
  
  if(omit.bad.cells){
    good.cells <- cell.counts[mm10>=mincount,cell.name]
    data.counts <- data.counts[cell.name %in% good.cells,]
  }
  data.ercc <- data.counts[gene %like% "ERCC-",]
  data.counts <- data.counts[!gene %like% "ERCC-",]
  
  if(omit.bad.genes){
    data.counts[,gene_total:=sum(count),by=gene]
    data.counts <- data.counts[gene_total>0,]
  }
  
  list(data.counts=data.counts, data.ercc=data.ercc,plt=plt) 
}

load.scde <- function(libloc){
  require(flexmix)
  require(multicore)
  require(boot)
  require(Cairo)
  ...
  require(caTools)
  require(Rcpp)
  require(RcppArmadillo, lib.loc = libloc)
  require(scde)
  
}

cast.for.scde <- function(data.counts){
  gene.counts <- dcast.data.table(data.counts, gene ~ cell.name, value.var="count",fun.aggregate=mean,fill=0)
  gene.counts <- as.data.frame(gene.counts)
  rownames(gene.counts)<-gene.counts[,1]
  gene.counts <- gene.counts[,2:ncol(gene.counts)]
  gene.counts
}
...
# this function is now defunct (slow). Use get.gene.cor instead
OLD.corr.analysis <- function(data.counts,minc,maxc,expression.values,method="spearman"){
  data <- filter.genes(data.counts,min_cells=1)
  cast.counts <- cast.to.df(data,expression.values = expression.values,annot="none")
  print(cast.counts[1:10,1:10])
  cast.counts <- cast.counts[which(apply(cast.counts,1,sd)>0),]
  cast.counts <- cast.counts[,which(apply(cast.counts,2,sd)>0)]
  maxcor <- data.frame(row.names=colnames(cast.counts), max=rep(NA, length(colnames(cast.counts))))
  
  cor.all.genes <- cor(cast.counts,method = method)
  for (i in 1:nrow(cor.all.genes)){
    cor.all.genes[i,i] <- NA
  }
  maxcor <- apply(cor.all.genes, 1, max, na.rm=T)
  mincor <- apply(cor.all.genes, 1, min, na.rm=T)
  for (i in 1:nrow(cor.all.genes)){
    cor.all.genes[i,i] <- 1
  }
  rm(cor.all.genes)
  maxcor <- as.data.frame(maxcor[order(maxcor) ]); colnames(maxcor) <- "max"
  mincor <- as.data.frame(mincor[order(mincor)]); colnames(mincor) <- "min"
  maxcor$gene <- rownames(maxcor)
  mincor$gene <- rownames(mincor)
  
  genes.corr <- maxcor[which(maxcor$max > maxc), ]
  genes.corr <- genes.corr[genes.corr$gene %in% mincor$gene[which(mincor < -minc)], ]
  genes.corr <- genes.corr[order(genes.corr$gene), ]
  
  data.genes.corr <- cast.counts[,genes.corr$gene]
  mat.cor <- cor(data.genes.corr,method=method)
  list(data.genes.corr,mat.cor,maxcor,mincor)
}

# this function is now defunct
OLD.get.mat.cor <- function(listcor,maxc=0.5,minc=0.5){
  maxcor <- listcor[[3]]
  mincor <- listcor[[4]]
  ...
  data.genes.corr <- listcor[[1]]
  data.genes.corr <- data.genes.corr[,genes.corr$gene]
  mat.cor <- cor(data.genes.corr)
  mat.cor
}
...
# do PCA and plot a heatmap of the (up to) 150 genes most correlated with each principal component,
# with the cell       s ordered by their position along the PCA dimension and the genes ordered by their contribution
# to each dimension
# data should have experiment colors as column 1, columns 2-N are gene expression values
dimplots.pca.wExptCols <- function(data,dir,suffix="",ncp=10,plot.sigs=T,max.genes=75,min.cells.detect=1,cexCol=0.6,scramble=F,expression.values="log10.cpm",genes.sig=NA,cex.sig=1,
                                   plot.gene.cor=T,sig.exptCol=T,n.cores=6,plot.sigHeatmaps=T){
  if(is.na(genes.sig)){
    genes.sig=max.genes
  }
  ...
  require(ggplot2)
  require(data.table)
  if(scramble){pcalist <- do.robpca.scramble(data,ncp = ncp,expression.values=expression.values); suffix="_scrambled"}
  else{pcalist <- do.robpca(data,ncp = ncp,expression.values=expression.values)}
  ...
  data.pca <- merge(data, cast.pca, by="cell.name")
  ...
  setkey(data)
  expt_cols <- unique(data[,.(cell.name,experiment_color)]); expt_cols <- as.data.frame(expt_cols)
  rownames(expt_cols)<-expt_cols$cell.name; expt_cols <- expt_cols["experiment_color"]
  
  out.sig <- data.table(cell.name=unique(data.pca[,cell.name]))
  out.sigscore <- data.table(cell.name=unique(data.pca[,cell.name]))
  # for every dimension, find genes significantly associated and plot heatmap
  ...
  pc.allscores=foreach(dim=dims,.combine='cbind') %dopar% {
    ...
    cast.heatmap <- cast.to.df(data.pca[gene %in% genes.use],expression.values=expression.values)
    cast.heatmap <- merge(expt_cols,cast.heatmap,by=0); rownames(cast.heatmap)<-cast.heatmap$Row.names; cast.heatmap<-cast.heatmap[,2:ncol(cast.heatmap)]
    dim.order <- unique(data.pca[,.(cell.name,get(dim))]); setnames(dim.order,c("cell.name",dim));setkeyv(dim.order,cols=dim)
    genes.order <- data.table(dim=loadings[gene %in% genes.use,get(dim)]); 
    genes.order[,gene:=loadings[gene %in% genes.use,gene]]; setkey(genes.order,dim)  
    cast.heatmap <- cast.heatmap[dim.order[,cell.name],c("experiment_color",genes.order[,gene])]
    
    hc.list=no.dendro.heatmap.wExptCols(cast.heatmap, fname = paste0(dir,dim,"_heatmap",suffix,".pdf"),width=20,height=12,cexCol=cexCol*1.1)
    
    
    
    if(plot.gene.cor){
      genes.cor <- loadings[,gene][1:100]; setorderv(loadings,dim,-1); genes.cor <- c(genes.cor,loadings[,gene][1:100])
      cast.cor <- cast.to.df(data[gene %in% genes.cor]); cast.cor <- cast.cor[,which(apply(cast.cor,2,sum)>0)]
      mat.cor <- cor(cast.cor)
      maxcor <- max(max(mat.cor[mat.cor!=1])); mincor <- min(min(mat.cor))
      maxcor <- max(abs(maxcor),abs(mincor))
      barb.cormap(mat.cor,fname = paste0(dir,dim,"_geneCorMap",suffix,".pdf"),width=10,height=10,method='complete',maxcor=maxcor,mincor = -maxcor,cex=0.25)
    }
    
    
    # generate signature value of top PC +/- correlated genes    
    cast.sig <- get.sig(data.pca, genes.pos,genes.neg)
    cast.sig <- merge(cast.sig, unique(data.pca[,.(cell.name,experiment,experiment_color)]))
    setkey(cast.sig)
    expt.cols <- unique(cast.sig[,.(experiment,experiment_color)]); 
    ...
    if(sig.exptCol){
      ...
      plt=ggplot(cast.pca, aes_string(x=dim,y="..count.."))+geom_histogram()+xlab(dim)+theme_bw()
      ggsave(plt,file=paste0(dir,dim,"_histogram",suffix,".pdf"),height=4,width=5)
    ...
    # plot heatmap of sig genes ordered by signature value
    setorderv(loadings,dim,-1); genes.pos.sig <- loadings[,gene][1:genes.sig]
    setorderv(loadings,dim,1); genes.neg.sig <- loadings[,gene][genes.sig:1]
    
    if(plot.sigHeatmaps){
      plot.sigHeatmap.wExptCols(data.pca,genes.pos,genes.neg,fname = paste0(dir,dim,"_sigHeatmap",suffix,".pdf"),width=15,height=10,cexCol=cexCol)
    ...
    # cells ordered by PC scores, genes ordered by clustering
    
    
    # plot 2d signature density
    plt=ggplot(cast.sig,aes(x=pos,y=neg))+stat_bin2d(bins=7)+thm+scale_fill_gradientn(colours=scale.farmhouse.rose)
    ggsave(plt,file=paste0(dir,dim,"_sigDensity",suffix,".pdf"),height=4,width=5)
    
    # plot signature histogram
    plt=ggplot(cast.sig,aes(pos-neg,..density..))+geom_histogram(binwidth=0.05,fill='grey50')+ geom_density(color='white')+theme_bw()+thm
    ggsave(plt,file=paste0(dir,dim,"_sigHist",suffix,".pdf"),height=4,width=5)
    
    cast.sig[,pc.score:=pos-neg];
    cast.out.sig <- cast.sig[,.(cell.name,pos,neg,pc.score)]
...
dimplots.pca.lowMem <- function(data,cell.info,dir,suffix="",ncp=10,plot.sigs=T,max.genes=30,min.cells.detect=1,cexCol=0.6,scramble=F,expression.values="log10.cpm",genes.sig=NA,cex.sig=1,
                                plot.gene.cor=F,sig.exptCol=T,n.cores=6,plot.sigHeatmaps=F,PCA.type = 'robust',min.numgenes = NA,
                                ng.subsample = NA,...){
  ...
  pc.allscores <- foreach(dim=dims,.combine='cbind') %dopar% {
    ...
    # cast.heatmap <- cast.to.df(data.genes.use,expression.values=expression.values)
    # cast.heatmap <- merge(expt_cols,cast.heatmap,by=0); rownames(cast.heatmap)<-cast.heatmap$Row.names; cast.heatmap<-cast.heatmap[,2:ncol(cast.heatmap)]
    # dim.order <- unique(cell.info[,.(cell.name,get(dim))]); setnames(dim.order,c("cell.name",dim));setkeyv(dim.order,cols=dim)
    # genes.order <- data.table(dim=loadings[gene %in% genes.use,get(dim)]); 
    # genes.order[,gene:=loadings[gene %in% genes.use,gene]]; setkey(genes.order,dim)  
    # cast.heatmap <- cast.heatmap[dim.order[,cell.name],c("experiment_color",genes.order[,gene])]
    # 
    ...
    # Turns out mean-centered and scaled look almost identical for relative positions of cells
    # calculate signatures a the sum of the mean-centered, unscaled values of the top genes (most similar to PC scores)
    # cast.mc <- get.sig(data.genes.use, genes.pos,genes.neg,scale = F,center = T)
    # setnames(cast.mc,c('cell.name','neg.mc','pos.mc'))
    # cast.sig=merge(cast.sig,cast.mc,by='cell.name')
    # gc()
    ...
    if(sig.exptCol){
      ...
      # plt=ggplot(cast.sig,aes(x=pos.mc,y=neg.mc,color=experiment))+geom_point()+
      #   scale_color_manual(values=expt2col)+theme_bw()+coord_fixed()
      # ggsave(plt,file=paste0(dir,dim,"_MCsigScatter",suffix,".pdf"),height=4,width=8)
      # 
      plt=ggplot(cast.pca, aes_string(x=dim,y="..count.."))+geom_histogram()+xlab(dim)+theme_bw()
      ggsave(plt,file=paste0(dir,dim,"_histogram",suffix,".pdf"),height=4,width=5)
      
      plt=ggplot(cast.sig,aes(x=pos,y=neg,color=experiment))+geom_density2d()+
        scale_color_manual(values=expt2col)+thm+coord_fixed()+facet_wrap(~experiment)+
        theme_black()+theme(strip.background=element_blank(),strip.text.x=element_blank())
      ggsave(plt,file=paste0(dir,dim,"_2dDensity",suffix,".pdf"),height=6,width=8)
      ...
      ggsave(plt,file=paste0(dir,dim,"_sigScatter",suffix,".pdf"),height=4,width=6)
      # 
      # plt=ggplot(cast.sig,aes(x=pos.mc,y=neg.mc))+geom_point()+
      #   theme_bw()+coord_fixed()
      # ggsave(plt,file=paste0(dir,dim,"_MCsigScatter",suffix,".pdf"),height=4,width=6)
      # 
      plt=ggplot(cast.pca, aes_string(x=dim,y="..count.."))+geom_histogram()+xlab(dim)+theme_bw()
      ggsave(plt,file=paste0(dir,dim,"_histogram",suffix,".pdf"),height=4,width=5)
      
      plt=ggplot(cast.sig,aes(x=pos,y=neg))+geom_density2d()+
        thm+coord_fixed()+theme_black()
      ggsave(plt,file=paste0(dir,dim,"_2dDensity",suffix,".pdf"),height=6,width=8)
    }
    ...
    # plot 2d signature density
    # plt=ggplot(cast.sig,aes(x=pos,y=neg))+stat_bin2d(bins=10)+thm+scale_fill_gradientn(colours=scale.farmhouse.rose)
    # ggsave(plt,file=paste0(dir,dim,"_sigDensity",suffix,".pdf"),height=4,width=5)
    # 
    # plot signature histogram
    plt=ggplot(cast.sig,aes(pos-neg,..density..))+geom_histogram(binwidth=0.05,fill='grey50')+ geom_density(color='white')+theme_bw()+thm
...
# input is matrix with rows = cells and columns = genes
# cell.info is data.table with cell.name and other info to plot
dimplots.pca.v3 <- function(data, cell.info, dir, suffix="", ncp=10, cells.use = character(0), max.genes=30, min.cells.detect=1,
                            scramble=F, expression.values="log10.cpm", n.cores=8, meta.features = character(0),
                            PCA.type = 'robust', min.numgenes = NA, ng.subsample = NA, make.gene.biplots = F,
                            make.pc.biplots = T,...){
  require(cowplot)
  ...
  cell.info <- as.data.table(cell.info %>% filter(cell.name %in% rownames(data)))
  
  # remove cells with less than minimum number of genes (if specified)
  if(!is.na(min.numgenes)){
    data = data[cell.info[num.genes > min.numgenes, cell.name], ]
  }
  
  # only subsample the number of genes for calculating PCs. 
  # Keep all genes for calculating scores
  if(!is.na(ng.subsample)){ 
    data.pca = subsample.numgenes(data, ng.min = ng.subsample)
  } else {data.pca = data}
  ...
  require(data.table)
  if(PCA.type == 'robust'){
    if(scramble){pcalist <- do.robpca.scramble(data.pca,ncp = ncp,expression.values=expression.values); suffix="_scrambled"}
    else{        pcalist <- do.robpca(data.pca,ncp = ncp,expression.values=expression.values,...)}
    
  } 
  else if(PCA.type == 'classical'){
    pcalist <- do.pca(data.pca,ncp = ncp,expression.values=expression.values,...)
  }
  ...
  # write loadings and scores to a csv
  write.csv(loadings,file=file.path(dir, paste0("PCloadings",suffix,".csv")),row.names=T)
  print("PC loadings file saved")
  gc()
  dims <- colnames(cast.pca)[1:(ncol(cast.pca)-1)]
  ...
  if(length(meta.features) > 0) {
    plt.meta = list()
    for(i in 1:length(meta.features)){
      plt.meta[[length(plt.meta)+1]] <- list()
    }
  ...
  pc.allscores <- foreach(dim=dims,.combine='cbind') %dopar% {
    setorderv(loadings,dim,-1)
    genes.pos <- loadings[,gene][1:max.genes]
    setorderv(loadings,dim,1)
    genes.neg <- loadings[,gene][max.genes:1]
    
    # generate signature value of top PC +/- correlated genes 
    # Turns out mean-centered and scaled look almost identical for relative positions of cells
    cast.sig <- get.sig(data, genes.pos, genes.neg, from.mat = T)[, c('pos', 'neg')]
    cast.sig$pc.score <- cast.sig$pos - cast.sig$neg
    colnames(cast.sig) <- c(paste0(dim,'.pos'),paste0(dim,'.neg'),paste0(dim,'.score'))

    if(make.pc.biplots) {
      p <- ggplot(cast.sig, aes_string(paste0(dim,'.pos'), paste0(dim,'.neg'))) +
        geom_point() +
        coord_fixed()
      save_plot(file.path(dir,paste0(dim,'_biplot.pdf')), p, base_height = 3.5)
    }
    
    as.matrix(cast.sig)
  }
  
  print(nrow(pc.allscores))
  print("Saving PC scores (default and signature)")
  pc.allscores=as.data.frame(pc.allscores)
  pc.allscores$cell.name <- rownames(data)
  pc.allscores <- as.data.table(pc.allscores)
  pc.allscores = merge(pc.allscores,cast.pca,by='cell.name')
  write.csv(pc.allscores,file=file.path(dir, paste0("PC_allscores",suffix,".csv")),quote=F,row.names=T)  
    
  if(length(meta.features) > 0){
    # make metadata biplots of PC pos/neg scores
    print("Making metadata plots")
    pc.meta.plot = merge(pc.allscores, cell.info[,c('cell.name',meta.features), with=F], by='cell.name')
    
    
    biplot.meta = list()
    for(i in 1:length(meta.features)) biplot.meta[[i]] = list()
    hist.meta = list()
    for(i in 1:length(meta.features)) hist.meta[[i]] = list()
    violin.meta = list()
    for(i in 1:length(meta.features)) violin.meta[[i]] = list()
    
    j = 1
    for(m in meta.features)  {
      for(i in 1:length(dims)) {
        biplot.meta[[j]][[i]] <- ggplot(pc.meta.plot,aes_string(x=paste0(dims[i],'.pos'),y=paste0(dims[i],'.neg'))) +
          coord_fixed() +
          # geom_point(shape = 1) +
          geom_point(aes_string(color=m), shape = 16)

        violin.meta[[j]][[i]]<-ggplot(pc.meta.plot, aes_string(x=m,y=paste0(dims[i],'.score'), fill = m))+
          geom_violin() +
          theme(legend.position = 'none', axis.text.x = element_text(angle = 45, hjust = 1)) +
          geom_boxplot(outlier.shape = NA, notch = T, fill=NA)
      }
      j = j + 1 
    }
    
    # save metadata plots
    print("Saving metadata plots")
    
    i = 1
    foreach(i = 1:length(meta.features)) %dopar% {
      
      p = plot_grid(plotlist = violin.meta[[i]], ncol = 3, align = 'hv')
      save_plot(file=file.path(dir, paste0(meta.features[i],"_Violin",suffix,".pdf")), p, ncol = 3, nrow = ceiling(n.pcs / 3))
      p = plot_grid(plotlist = biplot.meta[[i]], ncol = 3, align = 'hv')
      save_plot(file=file.path(dir, paste0(meta.features[i],"_Biplot",suffix,".pdf")), p, ncol = 3, nrow = ceiling(n.pcs / 3))
     
    }
  }
  
  # Make biplots of PC +/- scores colored by gene by top loading to each PC
  j = 1
  if(make.gene.biplots) {
    print("making gene biplots")
    pc.meta.plot = merge(pc.allscores, cell.info[,c('cell.name',meta.features), with=F], by='cell.name')
    foreach(dim=dims) %dopar% {
      
      setorderv(loadings,dim,-1)
      genes.pos <- loadings[,gene][1:15]
      setorderv(loadings,dim,1)
      genes.neg <- loadings[,gene][15:1]
      
      data.plot = melt(data[, c(genes.pos[1:15], genes.neg[1:15])], value.name = 'log10.cpm')
      setnames(data.plot, c('cell.name', 'gene', 'log10.cpm'))
      data.plot = merge(data.plot, pc.meta.plot[, c('cell.name',paste0(dim,'.pos'),paste0(dim,'.neg')), with = F], by = 'cell.name')
      
      plt=ggplot(data.plot,aes_string(paste0(dim,'.pos'),paste0(dim,'.neg')))+
        geom_point(size=1, shape = 1) +
        geom_point(aes(color=log10.cpm), size=1, shape = 16) +
        facet_wrap(~gene)+
        theme(strip.text=element_text(size=16),axis.title=element_text(size=16),legend.text=element_text(size=14)) +
      ...
      save_plot(file.path(dir,'biplots/',paste0(dim,"_geneBiplots",suffix,".pdf")), plt, base_height = 14)
    }
  }
...
run.ercc.pca <- function(data.ercc, dir, num.iter=20, ncp=6 ){
  # now redo analysis after removing high-mean erccs
  for(iter in 1:num.iter){
    suffix = paste0('iter',iter);
    require(reshape2, lib.loc = libloc)
    rm(data.ercc50x, data.ercc.i)
    data.ercc[,mean.cpm:=mean(log10.cpm),by=gene]
    data.ercc50x <- data.ercc[mean.cpm<2.5,.(cell.name,gene,log10.cpm)]
    for(i in 1:300){
      cast.ercc.i <- cast.to.df(data.ercc[mean.cpm<2.5])
      for(j in 1:ncol(cast.ercc.i)){
        cast.ercc.i[,j] <- cast.ercc.i[sample(nrow(cast.ercc.i)),j]
      }
      cast.ercc.i <- data.table(cast.ercc.i, keep.rownames = T); colnames(cast.ercc.i)[1]<-"cell.name"
      data.ercc.i <- melt(cast.ercc.i, id.vars = 1, measure.vars = 2:ncol(cast.ercc.i),variable.name = "gene",value.name = "log10.cpm") 
      data.ercc.i[,gene:=paste0(gene,".",as.character(i))]
      data.ercc50x <- rbindlist(list(data.ercc50x,data.ercc.i),use.names = T,fill = F)
    }
    
    data.ercc50x <- merge(data.ercc50x, unique(data.ercc[,.(cell.name,experiment,experiment_color)]),by="cell.name")
    
    # do rPCA on 6 dimensions and get a fold-change value for the two gene sets
    pcalist <- do.robpca(data.ercc50x,ncp = ncp)
    loadings <- pcalist[[3]]
    cast.pca <- pcalist[[2]]
    
    write.csv(cast.pca,file=paste(dir,"PCscores.csv",suffix,".csv"),quote=F,row.names=T)
    write.csv(loadings,file=paste(dir,"PCloadings.csv",suffix,".csv"),quote=F,row.names=T)
  }
...
#deprecated
dimplots.pca.signatureOrder <- function(data,dir,suffix="",ncp=10,min.p.val=1e-3,max.genes=75,min.cells.detect=1,cexCol=0.6){
  
  require(ggplot2)
  require(data.table)
  pcalist <- do.robpca(data,ncp = ncp)
  loadings <- pcalist[[3]]
  cast.pca <- pcalist[[2]]
  
  # write loadings and scores to a csv
  write.csv(cast.pca,file=paste0(dir,"PCscores",suffix,".csv"),quote=F,row.names=T)
  write.csv(loadings,file=paste0(dir,"PCloadings",suffix,".csv"),quote=F,row.names=T)
  
  #   print(unique(data[,gene]))
  data.pca <- merge(data, cast.pca, by="cell.name")
  #   print(unique(data.pca[,gene]))
  data.pca[,numcells_det:=sum(log10.cpm>0),by=gene]; data.pca <- data.pca[numcells_det > min.cells.detect]
  dims <- colnames(cast.pca)[1:(ncol(cast.pca)-1)]
  ...
  for(dim in dims){
    data.pca[,pval:=NULL]
    data.pca[,c("cor","pval"):=list(cor(get(dim),log10.cpm,method = "spearman"),cor.test(get(dim),log10.cpm,method="spearman")$p.value),by=gene]; setkey(data.pca)
    
    stats <- unique(data.pca[,.(gene,cor,pval)]); setkey(stats,pval,cor)
    
    genes.neg <- stats[cor<0]; genes.pos <- stats[cor>0]
    genes.neg <- genes.neg[pval < min.p.val]; genes.neg <- as.character(genes.neg[1:min(max.genes,nrow(genes.neg)),gene])
    genes.pos <- genes.pos[pval < min.p.val]; genes.pos <- as.character(genes.pos[1:min(max.genes,nrow(genes.pos)),gene])
    genes.use<-c(genes.pos,genes.neg);
    
    # generate signature value of PC-correlated + anticorrelated genes
    data.sig <- data.pca[gene %in% genes.use]
    ...
    cast.sig[,theta:=pos/(pos+neg)]
    histrange=max(cast.sig[,theta])-min(cast.sig[,theta]); numcells = length(unique(cast.sig[,cell.name]))
    setorder(cast.sig, theta);
    
    cast.heatmap <- cast.to.df(data.pca[gene %in% genes.use])
    cast.heatmap <- merge(expt_cols,cast.heatmap,by=0); rownames(cast.heatmap)<-cast.heatmap$Row.names; cast.heatmap<-cast.heatmap[,2:ncol(cast.heatmap)]
    
    genes.order <- data.table(dim=loadings[gene %in% genes.use,get(dim)]); 
    genes.order[,gene:=loadings[gene %in% genes.use,gene]]; setkey(genes.order,dim)  
    cast.heatmap <- cast.heatmap[cast.sig[,cell.name],c("experiment_color",genes.order[,gene])]
    
    hc.list=no.dendro.heatmap.wExptCols(cast.heatmap, fname = paste0(dir,dim,"_heatmap_sigOrder",suffix,".pdf"),width=20,height=12,cexCol=cexCol)
    
    # make heatmaps with rows colored by theta value
    #     colFun <- colorRamp(brewer.pal(11, "RdYlGn"))
    #     cell.name.stats <- cast.sig[,.(cell.name,theta)]
    #     cell.name.stats[,scaledtheta:=(theta-min(cell.name.stats[,theta]))/diff(range(cell.name.stats[,theta]))]
    #     
    #     setkey(cell.name.stats,scaledtheta)
    #     colmat <- colFun(cell.name.stats[,scaledtheta])
    #     thetacols <- rgb(colmat,maxColorValue = 256)
    #     names(thetacols) <- cell.name.stats[,cell.name]
    #     theta_cols <- data.frame(thetacols); colnames(theta_cols)<-"theta_color"
    #     
    #     cast.heatmap <- cast.to.df(data.pca[gene %in% genes.use])
    #     cast.heatmap <- merge(theta_cols,cast.heatmap,by=0); rownames(cast.heatmap)<-cast.heatmap$Row.names; cast.heatmap<-cast.heatmap[,2:ncol(cast.heatmap)]
    #     cast.heatmap <- cast.heatmap[cast.sig[,cell.name],c("theta_color",genes.order[,gene])]
    #     colnames(cast.heatmap) <- c("experiment_color",colnames(cast.heatmap)[2:ncol(cast.heatmap)])
    #     
    #     hc.list=no.dendro.heatmap.wExptCols(cast.heatmap, fname = paste0(dir,dim,"_heatmap_thetacols",suffix,".pdf"),width=20,height=12,cexCol=cexCol)
    #     
    #     plt2=ggplot(cast.sig, aes(x=pos, y=neg, color=cell.name))+geom_point()+theme_bw()+coord_fixed()+scale_color_manual(values=thetacols,guide=F)+
    #       xlab(paste(dim,"positive signature"))+ylab(paste(dim,"negative signature"))
    #     ggsave(plt2,file=paste0(dir,dim,"_signaturePlot",suffix,".pdf"),height=4,width=5)
    #     
    #     theta_dt <- data.table(theta_cols,keep.rownames = T); setnames(theta_dt,c("cell.name","theta_color"))
    #     cast.sig <- merge(cast.sig, theta_dt, by="cell.name")
    #     pdf(paste0(dir,dim,"_signaturePlot_thetaLegend",suffix,".pdf"),height=4,width=5)
    #     plot(cast.sig[,pos],cast.sig[,neg],color = cast.sig[,theta_color])
    #     legend.col(col = colorRampPalette(brewer.pal(11,"RdYlGn"))(51), lev = cast.sig[,theta])
    #     dev.off()
    
    
    # plot with experiment colors
    expt.cols <- unique(data.sig[,.(experiment,experiment_color)])
    setkey(expt.cols, experiment); expt.cols2 <- unique(expt.cols[,experiment_color]); names(expt.cols2) <- expt.cols[,experiment]
    
    plt2=ggplot(cast.sig, aes(x=pos, y=neg, color=experiment))+geom_point()+theme_bw()+coord_fixed()+scale_color_manual(name="experiment",values=expt.cols2)+
      xlab(paste(dim,"positive signature"))+ylab(paste(dim,"negative signature"))
    ggsave(plt2,file=paste0(dir,dim,"_signaturePlot_exptCols",suffix,".pdf"),height=4,width=5)
    
    histrange=max(cast.sig[,theta])-min(cast.sig[,theta]); numcells = length(unique(cast.sig[,cell.name]))
    plt3=ggplot(cast.sig, aes(x=theta, y=..count..))+geom_histogram(binwidth=8*histrange/numcells)+theme_bw()+xlab(paste(dim, "theta"))
    ggsave(plt3,file=paste0(dir,dim,"_signatureHisto.pdf",suffix,".pdf"),height=4,width=5)
...
dimplots.pca <- function(data,dir,ncp=10,min.p.val=1e-3,max.genes=75,min.cells.detect=1){
  ...
  # write loadings and scores to a csv
  write.csv(cast.pca,file=paste(dir,"PCscores.csv",sep="/"),quote=F,row.names=T)
  write.csv(loadings,file=paste(dir,"PCloadings.csv",sep="/"),quote=F,row.names=T)
  ...
  for(dim in dims){
    data.pca[,pval:=NULL]
    data.pca[,c("cor","pval"):=list(cor(get(dim),log10.cpm,method="spearman"),cor.test(get(dim),log10.cpm)$p.value,method="spearman"),by=gene]; setkey(data.pca)
    stats <- unique(data.pca[,.(gene,cor,pval)]); setkey(stats,pval,cor)
    genes.neg <- stats[cor<0]; genes.pos <- stats[cor>0]
    genes.neg <- genes.neg[pval < min.p.val]; genes.neg <- as.character(genes.neg[1:min(max.genes,nrow(genes.neg)),gene])
    genes.pos <- genes.pos[pval < min.p.val]; genes.pos <- as.character(genes.pos[1:min(max.genes,nrow(genes.pos)),gene])
    ...
    genes.order <- data.table(dim=loadings[gene %in% genes.use,get(dim)]); 
    genes.order[,gene:=loadings[gene %in% genes.use,gene]]; setkey(genes.order,dim)  
    cast.heatmap <- cast.heatmap[dim.order[,cell.name],genes.order[,gene]]
    
    hc.list=no.dendro.heatmap(cast.heatmap, fname = paste0(dir,dim,"_heatmap.pdf"),width=20,height=12,cexCol=0.5)
    plt=qplot(cast.pca[,get(dim)])+xlab(dim)
    ggsave(plt,file=paste0(dir,dim,"_histogram.pdf"),height=4,width=5)
  }
...
do.robpca.rank <- function(data.counts,min_cells=0,min_sd=0,ncp=10,expression.values="log10.cpm"){
  require(rrcov)
  data<-filter.genes(data.counts,expression.values,min_sd,min_cells)
  #   data[,expr.mean:=mean(get(expression.value)),by=.(gene)]
  #   data[,scaled.exp:=get(expression.value)/expr.mean]
  cast.counts <- cast.to.df(data,expression.values)
  cast.rnk <- as.data.frame(apply(cast.counts,2,rank))
  pca <- PcaHubert(cast.rnk, k = ncp)
...
do.rspca <- function(data.counts,min_cells=0,min_sd=0,ncp=10,expression.values="log10.cpm"){
  require(rrcovHD)
  data<-filter.genes(data.counts,expression.values,min_sd,min_cells)
  #   data[,expr.mean:=mean(get(expression.value)),by=.(gene)]
  #   data[,scaled.exp:=get(expression.value)/expr.mean]
  cast.counts <- cast.to.df(data,expression.values)
  pca <- SPcaGrid(cast.counts, k = ncp,method = "Qn",scale=T,trace=T,lambda=0.1)
...
# Matrices cannot be annotated as the purpose is for them to not require type-checking
...
group.mst <- function(mst){
  require(ggthemes, lib.loc = libloc)
  par(mfcol=c(4,4))
...
plot.ggpairs.wExptCols <- function(cell.info, dir='',num.pcs=6,suffix='',height=21,width=21,suffix.2='',sigType='PC',annot.expt=F,size=1,
                                   plt.pos.neg=F, plot.read.depth=T, feature.annot = 'experiment'){
...
# make biplots of gene expression signatures (requires two character vectors of genes)
...
bkg = element_rect(fill='black'); txt = element_text(color='white'); ln=element_line(color='white')
thm = theme(panel.background = bkg,panel.grid=element_blank(),legend.background=bkg,plot.background=bkg,axis.text=txt,
            axis.title=txt,axis.line=ln,axis.ticks=ln,legend.text=txt,legend.title=txt,legend.key=element_blank())

scale.cape.cod <- c('#2A4964','#6C91C0','#AFDCF2','#E8F9F6','#FDFFFD')

scale.autumn.prairie <- c('#2A2C31','#DFBA69','#FFFEDF')

scale.farmhouse.rose <- c('#2A4964','#655573','#C06C84','#F7B3BB','#FFEAE1')
...
make.facet.biplots <- function(data,pc.use,pc.dir,size=3.5,height=20,width=24,num.genes=15,pc.loadings.fname='PCloadings',pc.scores.fname='PC_allscores',
                               method.clust='pearson',ax.suf=c('.score','.score'),scores.dir=NA,print.dir=NA,scores.pc='',scale=F,custom.genes='',
                               suffix='', plot.pdf = T, fixedcoord = T, from.mat = T) {
